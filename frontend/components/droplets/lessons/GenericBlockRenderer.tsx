...
const GenericBlockRenderer: React.FC<GenericBlockRendererProps> = ({
  ...
  const [popup, setPopup] = useState<{ x: number; y: number; show: boolean; highlightSpan: HTMLElement | null; showColors: boolean }>({
    x: 0,
    y: 0,
    show: false,
    highlightSpan: null,
      ...
      highlights?.forEach(highlight => {
        ...
        while (walker.nextNode()) {
          ...
          if (currentPosition + nodeLength > highlight.position.start) {
            ...
            localEnd = Math.min(nodeLength, highlight.position.end - currentPosition);
            break;
          }
  ...
  const handleMouseUp = () => {
    ...
    if (text.length > 0 && contentRef.current) {
      ...
      const highlightSpan = startContainer.parentElement?.closest('span[style*="background-color"]') ||
                          endContainer.parentElement?.closest('span[style*="background-color"]');
      if (highlightSpan) {
        const parent = highlightSpan.parentNode;
        if (parent) {
          const highlightId = highlights.find(h => 
        ...
        setPopup({x: blockOffset - range.startOffset , y: blockOffset - range.endOffset, show: true, highlightSpan: highlightSpan as HTMLElement, showColors: false })
        return;
      }
  ...
  const handlePopupDelete = () => {
    if (popup.highlightSpan) {
      const highlightId = highlights.find(h => 
        ...
        h.color === (popup.highlightSpan as HTMLElement).style.backgroundColor
      )?.id;
      ...
      if (highlightId) {
        ...
        if (parent) {
          const textNode = document.createTextNode(popup.highlightSpan.textContent || '');
      ...
      >
        {popup.highlightSpan ? (
          ...
          <div style={{ display: "flex", gap: "5px" }}>
            <button onClick={() => handleApplyColor("red")} style={{ background: "red", width: "20px", height: "20px", border: "none", borderRadius: "50%" }} />
            <button onClick={() => handleApplyColor("yellow")} style={{ background: "yellow", width: "20px", height: "20px", border: "none", borderRadius: "50%" }} />
            <button onClick={() => handleApplyColor("blue")} style={{ background: "blue", width: "20px", height: "20px", border: "none", borderRadius: "50%" }} />
          </div>
        )}
          </>
        ) : (
