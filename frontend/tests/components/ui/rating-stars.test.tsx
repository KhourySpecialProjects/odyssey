...
describe('StarRating', () => {
  ...
  it('handles error when fetching rating', async () => {
    ...
    expect(consoleSpy).toHaveBeenCalledWith('Error fetching rating:', expect.any(Error));
  ...
  it('handles error when updating rating', async () => {
    ...
    (changeEnrollmentRating as jest.Mock).mockRejectedValue(new Error('Update failed'));
    ...
    expect(consoleSpy).toHaveBeenCalledWith('Error updating rating:', expect.any(Error));
    consoleSpy.mockRestore();
  });
});
})