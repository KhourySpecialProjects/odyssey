...
import { filterByDays, CHART_TIMEFRAMES_COMPACT } from "@/lib/chart-utils";
...
export function UniquePageviewBarChart({
  ...
  return (
    <div className="flex h-[280px] flex-col overflow-hidden rounded-[20px] bg-[#FCFCFD] dark:bg-slate-800 shadow-[0px_0px_4px_0px_rgba(0,0,0,0.25)]">
      <div className="flex items-start justify-between px-5 pt-4 pb-2">
        <p className="text-[20px] font-medium text-black dark:text-white">
          Daily Unique Pageviews
        </p>
        <TimeframeSelector options={CHART_TIMEFRAMES_COMPACT} value={timeframe} onChange={setTimeframe} />
      </div>
      <div className="min-h-0 flex-1 px-5 pb-4">
        {formatted.length === 0 ? (
          ...
          <ChartContainer config={chartConfig} className="h-full w-full">
            <BarChart data={formatted} margin={{ top: 15, right: 10, left: 0, bottom: 5 }}>
