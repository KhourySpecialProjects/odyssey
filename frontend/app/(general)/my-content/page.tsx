...
import { isContentCreator, isAuthorizedUserAdmin, isAuthorizedUserFaculty } from "@/lib/utils";
...
export default async function CreateRoute() {
  ...
  if (
    !user ||
    !user.email ||
    (!isAuthorizedUserAdmin(user.roles) && !isContentCreator(user.roles) && !isAuthorizedUserFaculty(user.roles))
  )
    ...
    <>
      ...
      <div className="s mx-auto mb-8 w-full max-w-7xl px-4 xl:p-0">
        <div className="flex w-full items-end justify-between">
          <h2 className="text-lg dark:text-slate-300 font-bold">My Droplets</h2>
        ...
        {(isContentCreator(user.roles) ||
          ...
          <>
            <div className="flex w-full items-end justify-between">
              <h2 className="mt-4 mb-2 text-lg dark:text-slate-300 font-bold">
                My Playlists
              </h2>
