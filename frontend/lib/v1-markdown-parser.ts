...
export function extractV1Metadata(markdown: string): V1Metadata {
  ...
  // Extract Authors
  const authorsMatch = metadataSection.match(
    /### Authors\s*\n((?:\*\s*.+\n?)+)/,
  );
  if (authorsMatch) {
  ...
  const descMatch = metadataSection.match(
    /### Description\s*\n<p>([\s\S]+?)<\/p>/,
  );
  if (descMatch) {
  ...
  const overviewMatch = metadataSection.match(
    /### Overview\s*\n<p>([\s\S]+?)<\/p>/,
  );
  if (overviewMatch) {
  ...
  const objMatch = metadataSection.match(
  ...
  // Extract Next Steps
  const nextStepsMatch = metadataSection.match(
    /### Next Steps\s*\n\*\s*(.+?)\s+linked to:\s+(.+)/,
  );
  if (nextStepsMatch) {
    metadata.nextSteps = [
    ...
    for (let i = 2; i < lines.length; i++) {
      ...
      // Answer line
        ...
        const answerMatch = line.match(
          /Answer:\s*<p>(.+?)<\/p>\s*is\s*(correct|incorrect)/,
        );
        if (answerMatch) {
