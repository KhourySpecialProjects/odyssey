...
export function FriendSearch({ authUsers, curUser }: FriendSearchProps) {
    ...
    const handleInputChange = (e: ChangeEvent<HTMLInputElement>) => {
        ...
        user.email?.substring(0, user.email?.length - 17).toLowerCase().includes(searchTerm.toLowerCase()) ||
                    ...
                    {searchResults.length > 0 ? (
                        <ul className="md:space-y-4 p-4">
                            {searchResults.map((user) => (
                                <FriendSuggestionsBlock suggUser={user} curUser={curUser}key={user.id} />
                            ))}
                        </ul>
                    ) : (
