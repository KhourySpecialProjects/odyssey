...
interface VoyageBoatProps {
  className?: string;
  size?: number;
}

// SVG paths extracted from frontend/public/circular_logo.svg
// The logo is 220x220 viewBox. We render just the 3 path elements (two sails + hull)
// in the brand color #297496, dropping the background circle.
export function VoyageBoat({ className, size = 1 }: VoyageBoatProps) {
  const width = Math.round(220 * size);
  const height = Math.round(220 * size);
