...
export function ExpandableEditor(
    {blocks, id, lessonId}: 
    {blocks: any, id: number, lessonId: number}) {
    const [content, setContent] = useState(blocks.find((b: any) => b.id === id).content);
    const [ title, setTitle ] = useState(blocks.find((b: any) => b.id === id).title);
    ...
    return (
        <div className="w-full p-4 border rounded-md border-slate-200">
            <div className="inline-flex flex-row items-center gap-2 font-bold text-sky-600 w-full">
                <Input className="mb-3" value={title} onChange={updateTitle} placeholder={"Expandable Title"}></Input>
                <ArrowDownFromLineIcon className="w-4 h-4 text-sky-400" />
            </div>
            <Separator />
            
            <TipTap className="mt-3" variant="lesson-expandable-body" initialContent={content} updateContent={updateContent} />
