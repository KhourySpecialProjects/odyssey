...
export function Sidebar({
  ...
  const { lessons, handleLessonReorder, updateLessons, isProcessing } = useLessonOrder(droplet);
  ...
  const handleLessonDelete = (lessonId: string) => {
    ...
    const newLessons = lessons.filter(lesson => lesson.id.toString() !== lessonId);
      ...
      <>
        ...
        <aside
            ...
            className={cn(
                ...
                expanded ? "md:translate-x-80 -transform-none" : "md:translate-x-0 -translate-x-full"
            )}
            aria-label="Sidebar"
        >
