...
export function GroupProgressGrid({ group }: GroupProgressGridProps) {
    ...
    useEffect(() => {
            ...
            console.log("group droplets: ", group.droplets);

            try {
                    (group.members || []).map(async (member) => {
                            (group.droplets || []).map(async (droplet) => {
                                const key = `${member.id}-${droplet.id}`;
                                const isComplete = (await getIsEnrollComplete(member.id, droplet.id))[0].isComplete;
                                //getIsEnrollComplete(member.id, droplet.id).then((enrollments) => {
                                 //   const isComplete = enrollments[0].isComplete;
                                  //   statuses[key] = isComplete;})
                                //const isComplete = (await getEnrollmentsByAuthorizedUser(member.id))[0].isComplete;
