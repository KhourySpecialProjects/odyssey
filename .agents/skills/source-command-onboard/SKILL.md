...
description: "Migrated source command `onboard`"
---
...
These come with the repo (registered in `.Codex/settings.json`). Check they
resolved:
...
```bash
Codex plugin install <name>@Codex-plugins-official --scope project 2>&1
```
...
```bash
Codex mcp add --transport http linear https://mcp.linear.app/mcp 2>&1
Codex mcp add --scope user --transport http figma https://mcp.figma.com/mcp 2>&1
```
...
Odyssey Onboarding
══════════════════
✓ Tooling — node 20.11.1, npm 10.2.4, docker 24.0.7, jq 1.7, Codex 1.x
