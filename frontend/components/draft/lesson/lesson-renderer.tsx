...
export function LessonRenderer({ lesson, dropletSlug }: LessonRendererProps) {
  ...
  const [lastSavedBlocks, setLastSavedBlocks] = useState<Block[]>(lesson.blocks);
  const lastSavedBlocksRef = useRef<Block[]>(lastSavedBlocks);
  const [name, setName] = useState(lesson.name);

  // Memoized update functions
  const updateBlocksBackend = useCallback(async (blocks: Block[]) => {
  ...
  const updateBlocksBackendReload = useCallback(async (blocks: Block[]) => {
    await updateLesson(lesson.id, { blocks }, { reload: true });
    console.log("Updated Blocks while reloading");
  }, [lesson.id]);

  const updateNameBackend = useCallback(async (name: string) => {
  ...
  const regenerateSlug = useCallback(async (name: string) => {
